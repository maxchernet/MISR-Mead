...
c
  ...
  111 l=l+1
     ...
     * xI(k,j)*(Gg(j)/2.+x/dL))/(Gg(j)/2.-x/dL)
      enddo
c      if(k.eq.m) print*, S(k,j), Q0m(k,j), Q1(k,j), xI(k,j), Gg(j), x, dL
      enddo
      xImt=0.
      do j=1,number/2
      x=xm+xr*points(j)
      xImt=xImt+weights(j)*xr*2.*Rs*abs(x)*xI(m+1,j)
c      print*, xImt, weights(j), xr, Rs, x, xI(m+1,j)
c      print*,j
      enddo
      do j=number/2+1,number
      xI(m+1,j)=xImt+xI1
...
c
     ...
     *abs(cos(teta_w))/G_w(j)
      ...
      enddo
      enddo
c      print*, 'xI1,xImt:', xI1, xImt
c      print*, 'x_rho_m=', x_rho_m(1,1)
c
